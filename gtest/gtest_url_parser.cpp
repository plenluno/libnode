...
TEST(GTestUrlParser, TestParseUrl1) {
    ...
    ASSERT_EQ(String::create(url->fragment)->compareTo(String::create("fr")), 0);
    parsed_url_free(url);
}

TEST(GTestUrlParser, TestParseUrl2) {
    struct parsed_url* url = parse_url("http://user1:passwd2@www.gtest.com:8888/foo/bar?abc=123&pqr=xyz#fr");
    ...
    ASSERT_EQ(String::create(url->path)->compareTo(String::create("foo/bar")), 0);
    ASSERT_EQ(String::create(url->query)->compareTo(String::create("abc=123&pqr=xyz")), 0);
    ASSERT_EQ(String::create(url->username)->compareTo(String::create("user1")), 0);
    ASSERT_EQ(String::create(url->password)->compareTo(String::create("passwd2")), 0);
    ASSERT_EQ(String::create(url->fragment)->compareTo(String::create("fr")), 0);
    parsed_url_free(url);
}
